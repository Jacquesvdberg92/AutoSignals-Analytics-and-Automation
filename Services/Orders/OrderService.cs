    ...
    private async Task CreateOrderForUser(Signal signal, UserData user, ProviderSettings settings, Dictionary<int, (string Name, decimal PricePrecision, decimal MinTradeUSDT, decimal AmountPrecision, int MinLeverage, int MaxLeverage)> precisions)
    {
        ...
        try
        {
            ...
            if (userBalance <= 0 && !settings.Testing)
            {
                ...
                await _errorLogService.LogErrorAsync($"User {user.Id} has insufficient balance. Exchange: {effectiveExchangeId}. Balance: {userBalance}",null, "OrderService.CreateOrderForActiveUsers");
                return;
            }
            ...
            if (tradeSizes["Entry"] <= 0)
            {
                ...
                await _errorLogService.LogErrorAsync($"Error calculating trade size for user {user.Id}. User Balance: {userBalance}. Signal: {signal}. Exchange: {user.ExchangeId}",null, "OrderService.CreateOrderForActiveUsers");
                return;
            }
            ...
            if (takeProfitOrders == null)
            {
                ...
                await _errorLogService.LogErrorAsync($"Error creating take profit orders for user {user.Id}", "OrderService.CreateOrderForActiveUser");
                return;
            }
        ...
        catch (Exception ex)
        {
            ...
            await _errorLogService.LogErrorAsync($"Error creating order for user {user.Id}: {ex.Message}", ex.StackTrace, "OrderService.CreateOrderForActiveUser");
