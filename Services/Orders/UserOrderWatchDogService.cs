    ...
    public class UserOrderWatchDogService
    {
        ...
        private async Task ProcessOrdersAsync()
        {
            using (var scope = _scopeFactory.CreateScope())
            {
                ...
                // Fetch pending orders so related orders (DCA, stoploss, TP) can be
                // transitioned from PENDING → OPEN when an entry order is executed.
                ...
                // Include pending orders so CreateOrUpdatePositionAsync can promote them to OPEN
                var ordersByKey = openOrders.Concat(pendingOrders)
