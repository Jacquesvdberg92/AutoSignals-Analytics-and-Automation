                    ...
                    foreach (var provider in providers)
                    {
                        ...
                        foreach (var user in users)
                        {
                            ...
                            if (providerSetting != null)
                            {
                                ...
                                if (providerSetting.TpPercentages.Count < tpCount)
                                {
                                    // Extend the list to match the provider's TP count,
                                    // preserving any percentages the user has already configured.
                                    while (providerSetting.TpPercentages.Count < tpCount)
                                    {
                                        providerSetting.TpPercentages.Add(25);
                                    }
                                    // Ensure the last TP is set to 100 if it hasn't been customised
                                    if (providerSetting.TpPercentages[tpCount - 1] == 0)
                                    {
                                        providerSetting.TpPercentages[tpCount - 1] = 100;
                                    }
                                }
                            }
                            else
