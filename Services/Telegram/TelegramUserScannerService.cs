    ...
    public class TelegramUserScannerService : BackgroundService
    {
        ...
        private async Task OnUpdatesAsync(UpdatesBase updates)
        {
            foreach (var update in updates.UpdateList)
            {
                ...
                var hasPhoto = msg.media is TL.MessageMediaPhoto { photo: TL.Photo };
                ...
                if (hasText)
                {
                    ...
                    {
                        _logger.LogError(ex, "TelegramUserScanner: Error processing text message from chat {ChatId}.", chatId);
                    }
                }

                // Process chart image
                if (hasPhoto && msg.media is TL.MessageMediaPhoto { photo: TL.Photo photo })
                {
                    try
                    {
                        using var photoStream = new MemoryStream();
                        await _client!.DownloadFileAsync(photo, photoStream);
                        var imageBytes = photoStream.ToArray();
                        if (imageBytes.Length > 0)
                            await _processor.ProcessImageMessageAsync(imageBytes, chatId, msg.date);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "TelegramUserScanner: Error processing photo message from chat {ChatId}.", chatId);
                    }
                }
            }
        }
