    ...
    public class KuCoinPriceService : IKuCoinService
    {
        ...
        public async Task<decimal> GetBalance(string apiKey, string apiSecret, string password)
        {
            try
            {
                ...
                while (retryCount < 3)
                {
                    response = await _futures.fetchBalance() as Dictionary<string, object>;
            ...
            {
                Console.WriteLine($"KuCoin GetBalance error: {ex.Message}");
            }
        ...
        public async Task<ExchangeOrderResult> SendEntryOrderAsync(Models.Order order, string apiKey, string apiSecret, string password)
        {
            try
            {
                ...
                // Determine contract size so we can convert base quantity → contracts
                double contractSize = 1.0;
                try
                {
                    var markets = await client.fetchMarkets() as List<object>;
                    var market = markets?
                    ...
                    if (market != null && market.TryGetValue("contractSize", out var csObj) && csObj != null)
                        contractSize = Convert.ToDouble(csObj, System.Globalization.CultureInfo.InvariantCulture);
                }
                catch { /* keep default */ }
                ...
                // KuCoin insufficient funds code 900014
                if (response != null && response.TryGetValue("code", out var codeObj) && codeObj?.ToString() == "900014")
                {
                    return new ExchangeOrderResult
                    {
                        Success = false,
                        ErrorCode = "40762",
                        ErrorMessage = response.ContainsKey("msg") ? response["msg"].ToString() : "Insufficient funds",
                        Response = response
                    };
                }
                // KuCoin minimum size code 900002
                if (response != null && response.TryGetValue("code", out var codeObj2) && codeObj2?.ToString() == "900002")
                {
                    return new ExchangeOrderResult
                    {
                        Success = false,
                        ErrorCode = "45110",
                        ErrorMessage = response.ContainsKey("msg") ? response["msg"].ToString() : "Order size below minimum",
                        Response = response
                    };
                }
                if (response != null && response.ContainsKey("message"))
            ...
            {
                Console.WriteLine($"KuCoin SendEntryOrderAsync error: {ex.Message}");
                await _errorLogService.LogErrorAsync($"KuCoin SendEntryOrderAsync error: {ex.Message}", ex.StackTrace, nameof(SendEntryOrderAsync), Newtonsoft.Json.JsonConvert.SerializeObject(order));
        ...
        public async Task<string> SendTakeProfitOrderAsync(Models.Order order, string apiKey, string apiSecret, string password)
        {
            try
            {
                ...
                // Determine contract size for this symbol
                double contractSize = 1.0;
                try
                {
                    var markets = await client.fetchMarkets() as List<object>;
                ...
                var contractsToClose = Math.Floor(baseQtyToClose / contractSize);
                if (contractsToClose <= 0)
                    return "Computed contracts to close is zero.";
                ...
                while (retryCount < 3)
                {
                    try
                    {
                        response = await client.createOrder(order.Symbol, "market", closeSide, contractsToClose, null, orderParams) as Dictionary<string, object>;
                        if (response != null && !response.ContainsKey("message"))
                            break;
                        var msg = response?.ContainsKey("message") == true ? response["message"]?.ToString() : null;
                        if (!string.IsNullOrEmpty(msg) && msg.Contains("Too many requests", StringComparison.OrdinalIgnoreCase))
            ...
            {
                Console.WriteLine($"KuCoin SendTakeProfitOrderAsync error: {ex.Message}");
                await _errorLogService.LogErrorAsync($"KuCoin SendTakeProfitOrderAsync error: {ex.Message}", ex.StackTrace, nameof(SendTakeProfitOrderAsync), Newtonsoft.Json.JsonConvert.SerializeObject(order));
        ...
        public async Task<string> SendStoplossOrderAsync(Models.Order order, string apiKey, string apiSecret, string password)
        {
            try
            {
                ...
                while (retryCount < 3)
                {
                    ...
                    {
                        lastEx = ex;
                        // KuCoin 400370 = no position to close
                        if (ex.Message.Contains("400370") || ex.Message.Contains("No position", StringComparison.OrdinalIgnoreCase))
                ...
                return response != null ? Newtonsoft.Json.JsonConvert.SerializeObject(response) : "No response from KuCoin";
            }
            catch (Exception ex)
            {
                Console.WriteLine($"KuCoin SendStoplossOrderAsync error: {ex.Message}");
                await _errorLogService.LogErrorAsync($"KuCoin SendStoplossOrderAsync error: {ex.Message}", ex.StackTrace, nameof(SendStoplossOrderAsync), Newtonsoft.Json.JsonConvert.SerializeObject(order));
                return $"Error: {ex.Message}";
            }
        }
    }
